...
static_assert(sizeof(long long) == 8, "Sanity check, see source code!");
template <> struct select_npy_type<long long> { const static NPY_TYPES type = NPY_INT64; };
static_assert(sizeof(unsigned long long) == 8, "Sanity check, see source code!");
...
bool eventplot(const std::vector<std::vector<Numeric>> &events, const std::map<std::string, std::string>& keywords = std::map<std::string, std::string>())
{
    ...
    for (size_t i = 0; i < events.size(); i++) {
        PyList_SetItem(list, i, detail::get_array(events[i]));
    }
